#!/usr/bin/env node
/**
 * AMC360 - Cloud Firestore Live Database Migrator (Node.js)
...
if (!keyPath) {
  ...
  // Scan Downloads for service account JSON (looks like serviceAccountKey*.json or contains firebase-adminsdk)
  if (fs.existsSync(DOWNLOADS_DIR)) {
    const files = fs.readdirSync(DOWNLOADS_DIR);
    for (const file of files) {
      if (file.endsWith('.json') && (file.includes('serviceAccount') || file.includes('firebase-adminsdk') || file.includes('service-management-system'))) {
...
if (!keyPath || !fs.existsSync(keyPath)) {
  ...
  console.log('  4. Once downloaded, simply re-run:');
  console.log('     node backend/firebase_firestore_migrate.js\n');
  console.log('='.repeat(65));
  process.exit(1);
}
  ...
  for (const collName of collNames) {
    ...
    for (let i = 0; i < docs.length; i += batchSize) {
      ...
      for (const doc of chunk) {
        const docRef = db.collection(collName).document(String(doc.id));
